<script>
	import {
		...
		reorganizeDataIfSmallScreenFunction,
		changeHeader,
		newHeader,
	...
	if (reorganizeData) {
		...
		dataArray = reorganizeDataIfSmallScreenFunction(dataArray);
		} else {
	...
	$: if (textToSearch !== '' && previoustextToSearch !== textToSearch) {
		...
		let searchDissertation = false;
				let searchExplication = false;
				if (search_items.includes('sujet-de-dissertation')) {
					searchDissertation = true;
					search_items.pop()
				}
				if (search_items.includes('explication-de-texte')) {
					searchExplication = true;
					search_items.pop()
				}
		if (desactivateRegexDefault === false) {
			...
			setTimeout(function () {
				...
				for (const row of rows) {
					...
					if (searchResults) {
						if (searchDissertation == false && searchExplication == false) {
							rowsFiltered = [...rowsFiltered, row];
						}  else {
							const sujet = row[row.length-1];
							const lastChar = sujet.charAt(sujet.length-1);
							if (searchDissertation) {
								if (lastChar=='?' && sujet.length < 300) {
									rowsFiltered = [...rowsFiltered, row];
								}
							}
							if (searchExplication) {
								if (lastChar!='?' || sujet.length > 300) {
									rowsFiltered = [...rowsFiltered, row];
								}
							}
		...
		<thead>
			<tr>
				{#each headers as header, i}
					<th class:small="{i<3}" data-key="{header}" on:click={() => sortColumnOnClick(i)}>{@html header}</th>
				{/each}
			...
			{#if innerWidth > 800 && activateFilters==true && desactivateRegexDefault==false && headersLength>1}
				<tr class="filters">
					{#each headers as header, i}
						<td class:small="{i<3}"><input type="text" id="filter-{i}" name="filter-{i}" bind:value={filters[i]} placeholder="filtre"></td>
					{/each}
				</tr>
			{/if}
		</thead>
			...
			{:else}
				...
				{#if rows.length !=0}
					{#each rows as row}
						<tr>
							{#each row as cell,i}
								<td class:small="{i<3}">{@html cell}</td>
							{/each}
						</tr>
					{/each}
				{:else}
